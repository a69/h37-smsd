...
void send_sms_callback (GSM_StateMachine *sm, int status, int MessageReference, void * user_data)
{
    ...
    if (status==0) {
        printf("..OK");
        sms_send_status = (GSM_Error volatile) ERR_NONE;
    } else {
        printf("..error %i", status);
        sms_send_status = (GSM_Error volatile) ERR_UNKNOWN;
    }
...
int main(int argc UNUSED, char **argv UNUSED)
{
    GSM_SMSMessage sms;
    GSM_SMSC PhoneSMSC;
    char recipient_number[] = "+989128485882";
    char message_text[] = "salam";
     ...
     * instant response
     */
    sms_send_status = (GSM_Error volatile) ERR_TIMEOUT;
